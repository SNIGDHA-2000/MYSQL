...
SELECT MIN(Population) AS MinPersonPopulation FROM Persons;
...
SELECT COUNT(*) AS NumberOfRowsInPersons FROM Persons;
...
SELECT Id, Country_name, Population, Area FROM Country ORDER BY RAND() LIMIT 3;

-- 10. List all persons ordered by their rating in descending order. 
SELECT Id, Fname, Lname, Rating FROM Persons ORDER BY Rating DESC;
 ...
 SELECT Country_name, COUNT(Id) AS NumberOfPersons, AVG(Rating) AS AverageRating FROM Persons GROUP BY Country_name HAVING COUNT(Id) > 2 ORDER BY AverageRating ASC;
